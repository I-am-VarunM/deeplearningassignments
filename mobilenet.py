...
class MobileNet(nn.Module):
    ...
    first_stage_stride : bool
       wehther stride is used at the first stage.
    ...
    def __init__(self,channels,first_stage_stride,in_channels=3,in_size=(224,224),num_classes=1000):
     ...
     for i,channels_per_stage in enumerate(channels[1:]):
         ...
         for j, out_channels in enumerate(channels_per_stage):
             stride =2 if(j==0) and ((i!=0) or frist_stage_stride) else 1
    ...
    def forward(self,x):
     ...
     x=self.output(x)
     return x
def get_mobilenet(width_scale, model_name = None,pretrained=False,root=os.path.join("-",".torch","models"),**kwargs):
    """
    Create Mobilenet model with specific parameters.
    Parameters:
    width_scale : float
      Scale factor for width of layers.
    model_name: str for None, default None
    ...
    net = MobileNet(channels = channels, first_stage_stride=first_stage_stride, **kwargs)

    if pretrained:
        if (model_name is None) or (not model_name):
            raise ValueError("Parameter 'model_name' shoudl be properly initialized for loading pretrained model.")
...
def mobilenet_w1(**kwargs):
    ...
    pretrained : bool,default False
      whether to load the pretrained weights for model.
    root : str, deafgult '~/torch/dpe;s'
       location for keeping the model parameters.
    """
    return get_mobiel(width_scale = 1.0, model_name = "mobilenet_w1", **kwargs)
...
def _test():
    import torch
    pretrained = False
    models = [mobilenet_w1,mobilenet_w3d4,mobile_wd2,mobilenet_wd4,]
    ...
    for model in models:
        ...
        assert (model!= mobilenet_wd34 or weight_count == 2585560)
